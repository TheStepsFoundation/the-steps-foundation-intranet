...
type Props = {
  ...
  buttonLabel?: string
  /** Which side the dropdown menu aligns to. Default 'right'. */
  ...
  return (
    <div ref={rootRef} className="relative inline-block">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
      >
        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
        </svg>
        <span>{buttonLabel ?? 'Columns'}</span>
        <span className="text-xs text-gray-500 dark:text-gray-400">({visibleCount}/{totalCount})</span>
      ...
      {open && (
        <div className={`absolute ${menuAlign === 'left' ? 'left-0' : 'right-0'} mt-1 w-72 max-h-[70vh] flex flex-col rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg z-40`}>
