...
// Shared admin top navigation — the single source of truth for the menu bar so
// the hub and every /students page render an identical header that can't drift.
// Auth state comes from useAuth; pages keep their own auth guards. The small
// ProfileAvatar appears on the right of every admin page (it links to /profile).
// Global intranet settings now live in the hub's "Globals" module, not a header
// gear; the mobile drawer keeps a labelled Settings link for convenience.
export default function AdminHeader() {
  const { user, signOut, teamMember } = useAuth()
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <nav className="hidden sm:flex items-center gap-1 text-sm">
              <Link href="/hub" className={`px-3 py-1.5 rounded-md ${pathname === '/hub' ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>Hub</Link>
              <NavDropdown label="Workflows" active={pathname?.startsWith('/tracker') ?? false} items={[
                { href: '/tracker', label: 'Task Tracker' },
                { href: '/tracker/strategy', label: 'Strategy' },
              ]} />
              <NavDropdown label="Students" active={pathname?.startsWith('/students') ?? false} items={[
